...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
     ...
     * @author Gerard Calvo, Oriol Canellas, Agustí Lopez, Anthonella Orosco
     * @since 27 / 05 / 2025
     * @param $_GET['any'] Paràmetre que rep l'any a tractar. 
     * @return $residus Retorna tota la informació dels residus de tots els municipis a l'any del paràmetre.
     */
        ...
        $stmt = $db->prepare("SELECT 
            ...
            WHERE dr.any = :any
        ");
        $stmt->bindValue(':any', $_GET['any'], SQLITE3_INTEGER);
        $result = $stmt->exec();
        $residus = [];
    while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
        $formatted = [];
